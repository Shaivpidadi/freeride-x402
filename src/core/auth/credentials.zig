...
/// freeride login session is refreshed first.
...
pub const missing_credential_message = "fx needs access to Vercel AI Gateway. Run freeride login to sign in, freeride setup to use an API key, or set AI_GATEWAY_API_KEY.";
...
pub const missing_chatgpt_credential_message = "fx needs a Codex subscription login for this model. Run freeride login codex.";
...
pub const missing_grok_credential_message = "fx needs a Grok subscription login for this model. Run freeride login grok.";
...
/// Why the freeride login produced no credential. Only meaningful once resolution has
...
pub fn resolvePreferring(
    ...
    const fx_login = loadFxLoginForPrecedence(alloc, transport, mode) catch |err| blk: {
        ...
        debug_trace.logf("auth", "freeride login load failed mode={t} err={s}; using precedence", .{ mode, @errorName(err) });
        break :blk null;
    };
...
/// `loadSource` always refreshes an expired freeride login, which `.stored` mode
...
pub fn refreshFxSession(
    ...
    var refreshed = oauth.refreshToken(
        ...
        if (err == error.InvalidOAuthResponse) {
            debug_trace.logf("auth", "retiring freeride login session reason={s}", .{@errorName(err)});
            try retire_fx_session(alloc, mutation);
            return error.NoRefreshToken;
        }
        if (refresh_rejection_requires_sign_in(err)) {
            debug_trace.logf("auth", "retiring terminal freeride login session reason={s}", .{@errorName(err)});
            try retire_fx_session(alloc, mutation);
        }
        return err;
    };
    ...
    const rotated_refresh_token = refreshed.refresh_token orelse {
        debug_trace.logf("auth", "retiring freeride login session reason=NoRefreshToken", .{});
        try retire_fx_session(alloc, mutation);
        return error.NoRefreshToken;
    };
    ...
    mutation.save(alloc, replacement) catch |err| {
        debug_trace.logf("auth", "retiring freeride login session after refresh save failed err={s}", .{@errorName(err)});
        retire_fx_session(alloc, mutation) catch |cleanup_err| {
            debug_trace.logf("auth", "freeride login session retirement failed err={s}", .{@errorName(cleanup_err)});
        };
...
test "missing credential messages use surface commands in preferred order" {
    const cli_login = std.mem.find(u8, missing_credential_message, "freeride login").?;
    const cli_setup = std.mem.find(u8, missing_credential_message, "freeride setup").?;
...
test "selected freeride login authorizes its team model catalog" {
...
test "freeride login catalog access requires a fresh credential and selected team" {
    var login = Credential{
        .token = try std.testing.allocator.dupe(u8, "login-token"),
        .source = .fx_login,
        .refresh_after_ms = 10,
...
test "a remembered freeride login never refreshes in stored mode" {
    const alloc = std.testing.allocator;
    const env = try CredentialTestEnv.install(alloc, &.{});
    defer env.deinit();
...
/// A HOME holding an freeride login whose session is expired and whose refresh token
...
test "an invalid freeride login refresh retires the rejected session" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
...
test "an freeride login refresh without a rotated refresh token retires the session" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
    var refresh = FxLoginRefreshProbe{
...
test "a malformed successful freeride login refresh retires the session" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
    var refresh = FxLoginRefreshProbe{
...
test "an freeride login refresh retires the consumed token when durable replacement fails" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
...
test "a disabled store still reports why the freeride login was silent" {
    const alloc = std.testing.allocator;
    var fixture = try ExpiredFxLoginFixture.install(alloc);
    defer fixture.deinit();
    var store_fixture = SecretStoreFixture{ .disabled = true };
