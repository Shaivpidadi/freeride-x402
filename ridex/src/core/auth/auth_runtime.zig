    ...
    pub fn missingHelp(self: StatusSnapshot, surface: MissingHelpSurface) ?[]const u8 {
        ...
        if (self.required_source == .fx_login) {
            return if (self.fx_login_status == .unavailable)
                "The saved freeride login could not be loaded. Run freeride login to repair this source; no other credential was selected."
            else
                "freeride login is selected but unavailable. Run freeride login to reconnect; no other credential was selected.";
        }
...
test "auth failure snapshot keeps refresh failures distinct from HTTP rejection" {
    ...
    const message = try snapshot.renderText(std.testing.allocator);
    defer std.testing.allocator.free(message);
    try std.testing.expectEqualStrings("freeride login credential refresh failed", message);

    const json = try snapshot.renderJson(std.testing.allocator);
    defer std.testing.allocator.free(json);
    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, json, .{});
    defer parsed.deinit();
    try std.testing.expectEqualStrings("freeride login", parsed.value.object.get("source").?.string);
...
test "auth runtime never admits a teamless freeride login credential" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
...
test "auth status keeps an unavailable explicit freeride login distinct from automatic absence" {
    const status = StatusSnapshot{
        .required_source = .fx_login,
        .fx_login_status = .unavailable,
    };
    const help = status.missingHelp(.cli).?;
    try std.testing.expect(std.mem.find(u8, help, "Run freeride login") != null);
...
test "auth status snapshot reports an expired session without claiming it is unrefreshable" {
    ...
    try std.testing.expectEqualStrings(
        "freeride login is configured; refreshable=true; team=vercel-labs",
        fresh_detail,
    );
    ...
    try std.testing.expectEqualStrings(
        "freeride login is configured; session expired; refreshable=true; team=vercel-labs",
        stale_detail,
    );
...
test "logout replaces an active freeride login with the next available source" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    runtime.skipOnboarding();
...
test "logout reconciliation adopts a newer concurrent freeride login" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
...
test "adopting freeride login publishes Vercel session availability to setup" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
