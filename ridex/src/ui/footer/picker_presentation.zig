...
test "partially visible auth picker shows a source window without duplicates" {
    ...
    try std.testing.expect(std.mem.find(u8, first_source.items, "freeride login") == null);

    var selected_source = try composeAuthPickerRow(alloc, view, 3, 4, 80);
    defer selected_source.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, selected_source.items, "freeride login") != null);
    ...
    try std.testing.expect(std.mem.find(u8, scrolled_first.items, "freeride login") != null);
