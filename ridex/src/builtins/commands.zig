...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        .kind = .provider,
        .token = "provider",
        .usage = "provider <freeride|gateway|codex|grok>",
...
pub const top_level_help_groups = [_]TopLevelHelpGroup{
    ...
    .{ .entries = &.{
        ...
        .{ .kind = .provider, .usage = "provider <freeride|gateway|codex|grok>", .summary = "Choose the active model provider" },
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .paid, .command = "/paid", .help_entry = "/paid [on|off]", .completion_description = "pay for every request on Hedera instead of trying free first", .presentation_category = .account, .has_args = true, .accepts_payload = true },
